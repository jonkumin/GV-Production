...
export interface CacheOptions {
     ...
     * off. This also turns off queing of potentially cachable requests that platform performs
     * automatically.
     *
     * Note: if know for sure that a specific `GET` route will never cache then you should
     * always set `edge` caching to `false` for it as it will disable `GET` request coalesing
...
export interface BrowserCacheOptions {
    ...
    /**
     * The method for requests that can be cached. Defaults to `'get'`.
     * This is generally used when using the Prefetcher to cache POST requests using
     * `router.get('/my/post/path')`. In this case, the route needs to be specified
     * with `.get` to correctly prefetch, but the browser cache needs specify `asMethod`
     * to be the method that the origin server expects.
     */
    asMethod?: HTTPMethod;
}
